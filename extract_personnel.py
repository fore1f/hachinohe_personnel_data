...
def main():
    ...
    for index, row in df.iterrows():
        ...
        if colA:
            ...
            if is_recruitment:
                # 採用・再任用の場合は（採用）等の形式にするため、旧職欄を上書き
                label = "採用" if "採用" in colA else "再任用"
                formatted_old_job = f"（{label}）"
            ...
            if colA == "退職":
                ...
                line = f"退職・{retirement_date_str}{c_part}{formatted_old_job}{colD}"
                output_lines.append(line)
            elif is_recruitment:
                # 採用・再任用の出力形式
                line = f"{display_bu}{colC}{formatted_old_job}{colD}"
                output_lines.append(line)
            else:
                if colA not in basic_idou:
                    ...
                    if display_colA == "出向":
                        ...
                        line = f"{_bu}{colC}へ出向{formatted_old_job}{colD}"
                    else:
                        line = f"{_bu}{colC}・{display_colA}{formatted_old_job}{colD}"
                    output_lines.append(line)
                else:
                    # 基本の抽出
                    line = f"{display_bu}{colC}{formatted_old_job}{colD}"
